...
class Encoder(torch.nn.Module):
    ...
    def forward(self, fixed_embedding, dynamic_embedding, pair, pos14, atom_mask, mut_pos):
        ...
        x = self.fixed_embedding_transform(torch.cat((self.esm2_transform(dynamic_embedding), fixed_embedding[:, :, :-1]), dim = -1))
        x = self.finetune_p(torch.cat((x, fixed_embedding[:, :, -1:]), dim = -1))
...
class PretrainModel(torch.nn.Module):
    ...
    def forward(self, wt, mut):
        ...
        wt_Tm = self.encoder(torch.cat((wt['fixed_embedding'][:, :, :-2], wt_node_feat, wt['fixed_embedding'][:, :, -2:-1]), dim = -1), wt['dynamic_embedding'], wt['pair'], wt['pos14'], wt['atom_mask'], wt['mut_pos'])
        mut_Tm = self.encoder(torch.cat((mut['fixed_embedding'][:, :, :-2], mut_node_feat, mut['fixed_embedding'][:, :, -2:-1]), dim = -1), mut['dynamic_embedding'], mut['pair'], mut['pos14'], mut['atom_mask'], mut['mut_pos'])
        ...
        return (mut_Tm - wt_Tm) * self.finetune_rmse_coef + self.finetune_rmse_bias
