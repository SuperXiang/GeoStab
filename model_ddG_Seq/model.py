...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, node_dim, n_head, pair_dim, num_layer, dropout):
        super().__init__()
        ...
        self.fixed_embedding_transform = torch.nn.Sequential(
            torch.nn.Linear(node_dim + 12 + 64, node_dim),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(node_dim, node_dim)
        )
    ...
    def forward(self, fixed_embedding, dynamic_embedding, pair, pos14, atom_mask, mut_pos):
        
        # pos14 = [N, L, 14, 3]
        # atom_mask = [N, L, 14]
        # pair = [N, L, L, 7]
        
        x = self.fixed_embedding_transform(torch.cat((self.esm2_transform(dynamic_embedding), fixed_embedding[:, :, :-2]), dim = -1))
        x = self.finetune_pt(torch.cat((x, fixed_embedding[:, :, -2:]), dim = -1))
        ...
        x = self.dG_readout(x).squeeze(-1) * mut_pos
        return x.sum(1)

class PretrainModel(torch.nn.Module):
    
    def __init__(self, node_dim, n_head, pair_dim, num_layer, dropout, dms_node_dim, dms_num_layer, dms_n_head, dms_pair_dim):
        super().__init__()
        ...
        self.encoder = Encoder(node_dim, n_head, pair_dim, num_layer, dropout)
        self.finetune_rmse_coef = torch.nn.Parameter(torch.tensor([1.0], requires_grad = True))
        self.finetune_rmse_bias = torch.nn.Parameter(torch.tensor([0.0], requires_grad = True))
    ...
    def forward(self, wt, mut):
        ...
        wt_dG = self.encoder(torch.cat((wt['fixed_embedding'][:, :, :-3], wt_node_feat, wt['fixed_embedding'][:, :, -3:-1]), dim = -1), wt['dynamic_embedding'], wt['pair'], wt['pos14'], wt['atom_mask'], wt['mut_pos'])
        mut_dG = self.encoder(torch.cat((mut['fixed_embedding'][:, :, :-3], mut_node_feat, mut['fixed_embedding'][:, :, -3:-1]), dim = -1), mut['dynamic_embedding'], mut['pair'], mut['pos14'], mut['atom_mask'], mut['mut_pos'])
        
        # pred dms logits
        # logits_pred = self.pretrain_mlp(wt_node_feat)
        # logits_pred = self.logits_coef[None, :, None, None] * torch.cat((logits_pred.unsqueeze(1), wt['logits']), dim = 1)
        # logits_pred = logits_pred.mean(1)
        
        return (mut_dG - wt_dG) * self.finetune_rmse_coef + self.finetune_rmse_bias
